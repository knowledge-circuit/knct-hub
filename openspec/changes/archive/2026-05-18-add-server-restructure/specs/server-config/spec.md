...
- **WHEN** `DATABASE_URL=postgresql+asyncpg://u:p@localhost/knct` is set before startup
...
- **WHEN** `DATABASE_URL` is a `sqlite+aiosqlite://` URL
- **THEN** the server starts, runs migrations, and serves requests against that SQLite file

#### Scenario: Postgres URL connects
- **WHEN** `DATABASE_URL` is a `postgresql+asyncpg://` URL pointing at a running Postgres
