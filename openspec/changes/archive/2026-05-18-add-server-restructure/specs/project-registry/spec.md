...
The system SHALL identify each project by a unique kebab-case slug. The slug SHALL be supplied by the client in every hook request via an `X-Project-Slug` HTTP header sent to `POST /api/v1/hook`.
...
- **WHEN** a client POSTs `{ "slug": "existing" }` to `/api/v1/projects` and `existing` already exists
- **THEN** the response is HTTP 409 and no row is changed

#### Scenario: Invalid slug rejected
- **WHEN** a client POSTs `{ "slug": "Bad Slug!" }` to `/api/v1/projects`
